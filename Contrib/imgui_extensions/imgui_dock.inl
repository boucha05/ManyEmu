...
struct DockContext
{
    ...
    template <typename TSerialized, typename TNative>
    void serialize(ISerializer& serializer, TNative& value, const char* name)
    {
        TSerialized serializableValue = static_cast<TSerialized>(value);
        serializer.serialize(serializableValue, name);
        value = static_cast<TNative>(serializableValue);
    }

    void serialize(ISerializer& serializer, char*& value, const char* name)
    {
        size_t size = value ? static_cast<uint32_t>(strlen(value)) : 0;
        serializer.serializeTextSize(size, name);
        if (serializer.isReading())
        {
            if (value)
                MemFree(value);
            value = static_cast<char*>(MemAlloc(size + 1));
            value[size] = 0;
        }
        serializer.serializeTextData(value, name);
    }

    void serialize(ISerializer& serializer, Dock*&value, const char* name)
    {
        if (serializer.isReading())
        {
            int index;
            serialize<int32_t>(serializer, index, name);
            value = getDockByIndex(index);
        }
        else
        {
            int index = getDockIndex(value);
            serialize<int32_t>(serializer, index, name);
        }
    }
    ...
    void serialize(ISerializer& serializer)
    {
        ...
        serialize<int32_t>(serializer, version, "version");
        assert(version == CurrentVersion);

        size_t docksSize = m_docks.size();
        if (serializer.serializeSequenceBegin(docksSize, "docks"))
        {
            if (serializer.isReading())
            {
                while (docksSize-- != 0)
            ...
            for (int i = 0; i < m_docks.size(); ++i)
            {
                serializer.serializeSequenceItem();

                Dock& dock = *m_docks[i];
                auto location = ImStrdup(dock.location);
                serialize(serializer, dock.label, "label");
                serialize<int32_t>(serializer, dock.pos.x, "pos_x");
                serialize<int32_t>(serializer, dock.pos.y, "pos_y");
                serialize(serializer, location, "location");
                serialize<int32_t>(serializer, dock.size.x, "size_x");
                serialize<int32_t>(serializer, dock.size.y, "size_y");
                serialize<int32_t>(serializer, dock.status, "status");
                serialize<bool>(serializer, dock.active, "active");
                serialize<bool>(serializer, dock.opened, "opened");
                serialize(serializer, dock.prev_tab, "prev");
                serialize(serializer, dock.next_tab, "next");
                serialize(serializer, dock.children[0], "child0");
                serialize(serializer, dock.children[1], "child1");
                serialize(serializer, dock.parent, "parent");
            ...
            serializer.serializeSequenceEnd();
        }
    }
};
...
void SerializeDock(ISerializer& serializer)
{
    g_dock.serialize(serializer);
}
